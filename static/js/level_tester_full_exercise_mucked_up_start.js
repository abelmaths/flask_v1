...
function bind_check_button(question_number) {
	...
	$('#check_or_continue').click(function(){
		...
		// If not finished the question yet
		...
		// If an actual question end, update the scores and increment everything
		else {
...
function prepare_continue_button() {
	...
	// Check if mid-question (i.e. scaffold) - if so, we don't do much but continue same question
	if (last_line_answered()=="false") {
	...
	// If this is a continue button of a worked example, we send progress to server when clicked
	else if(['start','worked'].indexOf(display_format) !== -1) {
		console.log("It's a worked example, so we bind the continue button to 'next question'")
