...
Save progress after each question
	...
	3) Only allow continue once info is saved (see if acceptable)
...
Within app.py, get_level_blob_and_save_progress should save their progress to the user
...
Create randomisation of questions
...
function update_level_score_tracker() {
	// Keep track of scores which will then be sent to server at end of level

	level_score_tracker.push(
		{
			'qu:':question_number,
...
function prepare_continue_button() {
	...
	// Otherwise go to next question or level or finish homework
		...
		update_score_counters()
		update_progress_section()
		update_level_score_tracker()
		// Check what stage we are at (end_of_question, end_of_level, end of homework) - affects button funcitonality
		stage = check_stage()
		console.log("Going to save progress to server...")
		console.log("question number", question_number)
		console.log("level number", level_number)
		console.log(questions_correct_total_count, questions_incorrect_total_count, questions_correct_level_count, questions_incorrect_level_count)
...
function get_level_from_db_and_start_level(this_level_hash, next_level_hash) {
	// Provides server with level_id, will return json
	debug("Calling server to save progress and get the next level")
	debug("This is the progress we are sending:")
	debug(level_score_tracker)
	$.ajax( {
      url: '/_get_level_blob_and_save_progress',
      data: JSON.stringify ({
        'this_level_hash':this_level_hash,
        'next_level_hash':next_level_hash,
        'level_score_tracker':level_score_tracker
      }, null, '\t'),
...
var display_format = 'start'
var level = ''
var level_score_tracker = []
var hint_used = 'hint'

//var level_hash_list = from html template via Jinja
var max_level_number = (level_hash_list.length)-1
var level_number = -1
get_level_from_db_and_start_level('0',level_hash_list[level_number+1])
